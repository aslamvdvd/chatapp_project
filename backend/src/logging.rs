...
pub fn init_logging() -> Result<(WorkerGuard, WorkerGuard, WorkerGuard), Error> {
    ...
    let (system_writer, system_guard) = 
        tracing_appender::non_blocking(tracing_appender::rolling::never(daily_log_dir.clone(), "system_logs.jsonl"));
    let system_layer = tracing_subscriber::fmt::layer()
        ...
        .with_filter(EnvFilter::new("info,chatapp_by_aarchangel_backend[system_events]=trace,system_events=trace"));
    ...
    let (user_writer, user_guard) = 
        tracing_appender::non_blocking(tracing_appender::rolling::never(daily_log_dir.clone(), "user_logs.jsonl"));
    let user_layer = tracing_subscriber::fmt::layer()
        .json()
        .with_writer(user_writer)
        .with_span_events(FmtSpan::CLOSE)
        .with_filter(EnvFilter::new("info,chatapp_by_aarchangel_backend[user_events]=trace,user_events=trace"));

    // --- Admin Logs --- (Example, adjust target name as needed)
    let _admin_log_file = daily_log_dir.join("admin_logs.jsonl");
    let (admin_writer, admin_guard) = 
        tracing_appender::non_blocking(tracing_appender::rolling::never(daily_log_dir.clone(), "admin_logs.jsonl"));
    let admin_layer = tracing_subscriber::fmt::layer()
        .json()
        .with_writer(admin_writer)
        .with_span_events(FmtSpan::CLOSE)
        .with_filter(EnvFilter::new("info,chatapp_by_aarchangel_backend[admin_events]=trace,admin_events=trace"));

    // General console layer for fallback/other logs, or for development
    let _console_env_filter = EnvFilter::try_from_default_env()
    ...
    tracing_subscriber::registry()
        ...
        .try_init()
        .map_err(|e| Error::new(ErrorKind::Other, format!("Failed to initialize logger: {}", e)))?;
